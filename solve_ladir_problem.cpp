...
unsigned long stopMotorsPreviousMillis = 0;
unsigned long rotateMotorPreviousMillis = 0;
unsigned long rotateMotorDelayMillis = 200;  // Example delay time
...
void loop() {
  ...
  if (!isInitialized) {
    stopMotors();
    isInitialized = true;
    stopMotorsPreviousMillis = millis();  // Start the timer for stopMotors
  }

  if (isInitialized && (millis() - stopMotorsPreviousMillis >= 700)) {
    int enc1Pos, enc2Pos;
...
void forward_with_correction(int &motor1Speed, int &motor2Speed) {
  ...
  if (bothWheels_flag_count > 4) {
    ...
    rotateMotorPreviousMillis = millis();  // Start the timer for rotateMotor delay
    while (millis() - rotateMotorPreviousMillis < 130) {
      // Wait for 130 ms
    }
  }
  ...
  if (abs(diff) >= 10) {
    ...
    motor2Speed = 95; // Maintain speed of motor 2
  }
  // Apply the adjusted speeds to the motors
  ledcWrite(PWM_CHANNEL_1, motor1Speed);
  ledcWrite(PWM_CHANNEL_2, motor2Speed);
...
void rotateMotor1ToRight() {
  ...
  unsigned long rotateStartMillis = millis();
  ...
  while (!motor1Stopped) {
    ...
    if (leftWheels_flag_count > 4 && motor1Speed != 0 && motor2Speed == 0) {
      ...
      rotateMotorPreviousMillis = millis();  // Start the timer for rotateMotor delay
      while (millis() - rotateMotorPreviousMillis < rotateMotorDelayMillis) {
        // Wait for 200 ms
      }
    }
    ...
    if (enc1Pos < 970) {
      ...
      // Wait for 0.5 seconds using millis
      unsigned long stopStartMillis = millis();
      while (millis() - stopStartMillis < 500) {
        // Wait for 500 ms
      }
...
void turnAround() {
  ...
  while (!turnComplete) {
    ...
    if (leftWheels_flag_count > 4 && motor1Speed != 0 && motor2Speed != 0) {
      ...
      rotateMotorPreviousMillis = millis();  // Start the timer for rotateMotor delay
      while (millis() - rotateMotorPreviousMillis < 100) {
        // Wait for 100 ms
      }
    }
...
void stopMotors() {
  ...
  stopMotorsPreviousMillis = millis();  // Start the timer for stopMotors
  while (millis() - stopMotorsPreviousMillis < 300) {
    // Wait for 300 ms
  }
}
